...
class ODST(ModuleWithInit):
    ...
    def forward(self, input):
        ...
        feature_values = torch.einsum('bi,ind->bnd', input.double(), feature_selectors.double())
        ...
        bin_matches = torch.einsum('btds,dcs->btdc', bins.double(), self.bin_codes_1hot.double())
        # ^--[batch_size, num_trees, depth, 2 ** depth]

        response_weights = torch.prod(bin_matches, dim=-2)
        # ^-- [batch_size, num_trees, 2 ** depth]

        response = torch.einsum('bnd,ncd->bnc', response_weights.double(), self.response.double())
